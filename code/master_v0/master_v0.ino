...
float wheelRadius = 1;             // Bán kính bánh xe (m)
...
void setup() {
  ...
  xTaskCreatePinnedToCore(TaskReadSensor, "TaskReadSensor", 2048, NULL, 4, NULL, 1);
  ...
  xTaskCreatePinnedToCore(checkSIMTask, "CheckSIMTask", 2048, NULL, 1, NULL, 1);
}
...
void checkSIMTask(void* pvParameters) {
  while (true) {
    simSerial.println("AT+CPIN?");   // Gửi lệnh AT kiểm tra trạng thái SIM
    vTaskDelay(pdMS_TO_TICKS(100));  // Đợi phản hồi

    String aresponse = "";

    // Đọc phản hồi từ module SIM
    while (simSerial.available()) {
      aresponse += char(simSerial.read());
    }

    // Kiểm tra trạng thái SIM
    if (aresponse.indexOf("+CME ERROR: SIM not inserted") != -1) {
      Serial.println("SIM not inserted. Resetting SIM module...");
      Serial.println(sendATCommand("AT+CFUN=0"));
      Serial.println(sendATCommand("AT+CFUN=1,1"));
    }
    vTaskDelay(pdMS_TO_TICKS(10000));  // Đợi 10 giây trước khi kiểm tra lại
  }
}
...
String generateReportMessage() {
  ...
  if (gps.location.isValid()) {
    reportMessage += "- Location: www.google.com/maps?q=" + String(gps.location.lat(), 6) + "," + String(gps.location.lng(), 6) + "\n";
...
String generateAlertMessage() {
  ...
  alertmessage += "- Overspeeding: " + String(overspeedCount) + "\n";
