...
User users[] = {
  { { 0x03, 0xCD, 0x7A, 0x29 }, "Nguyen Van A", false },
  { { 0x93, 0xE7, 0x46, 0x16 }, "Nguyen Van B", false }
};
...
void setup() {
  ...
  xTaskCreate(taskSDLogger, "SD Logger", 5000, NULL, 2, &taskSDHandle);
  xTaskCreatePinnedToCore(taskRFID, "RFID Task", 8000, NULL, 3, NULL, 0);
  xTaskCreate(taskRFIDLED, "LED RFID Task", 1024, NULL, 1, NULL);
  xTaskCreate(taskGSMLED, "LED GSM Task", 1024, NULL, 1, NULL);
  xTaskCreate(taskSDLED, "LED SD Task", 1024, NULL, 1, NULL);
  xTaskCreatePinnedToCore(readGPSTask, "Read GPS Task", 4096, NULL, 2, NULL, 1);
  xTaskCreate(listenForMessages, "Listen for Messages", 6000, NULL, 4, NULL);
  xTaskCreatePinnedToCore(TaskReadSensor, "TaskReadSensor", 2048, NULL, 3, NULL, 0);
  xTaskCreatePinnedToCore(buzzerCollisionTask, "buzzerCollisionTask", 2048, NULL, 3, NULL, 1);
  xTaskCreatePinnedToCore(sendOverspeedWarningTask, "OverspeedWarningTask", 2048, NULL, 3, NULL, 1);
  xTaskCreatePinnedToCore(calculateSpeedTask, "Calculate Speed", 4096, NULL, 4, NULL, 0);  // Task tính tốc độ
  xTaskCreatePinnedToCore(drivingTimerTask, "DrivingTimerTask", 2048, NULL, 2, NULL, 1);
  xTaskCreatePinnedToCore(vehicleStatusTask, "Vehicle Status Task", 4096, NULL, 3, NULL, 0);
  xTaskCreate(readBatTask, "readBatTask", 2048, NULL, 2, NULL);
  xTaskCreate(readinputVoltageTask, "readinputVoltageTask", 2048, NULL, 1, NULL);
  xTaskCreate(buzzerstoleTask, "buzzerstoleTask", 2048, NULL, 2, NULL);
  xTaskCreate(buzzerOverspeedTask, "buzzerOverspeedTask", 2048, NULL, 3, NULL);
  xTaskCreate(resetTask, "ResetTask", 1000, NULL, 1, NULL);            // Tạo task reset
  xTaskCreate(debugprintTask, "debugprintTask", 2000, NULL, 1, NULL);  // Tạo task reset
}
  ...
  while (true) {
    ...
    if ((!vehiclestatus && !anyLoggedIn && speed > 10.0) || (vehiclestatus && !anyLoggedIn && speed != 0)) {
      if (!alertActive) {
        ...
        // Kích hoạt buzzer thông qua mutex
        // if (xSemaphoreTake(buzzerMutex, 0)) {
        //   xTaskCreatePinnedToCore(buzzerstoleTask, "Buzzer Stole Task", 2048, NULL, 1, NULL, 0);
        // }
      }
    } else {
      alertActive = false;
...
void taskSDLogger(void* parameter) {
  for (;;) {
    ...
    if (!isSDInserted()) {
      ...
      ledIntervalSD = 1000;
      // Xác định tên người dùng và trạng thái
      // String userName = "N/A";           // Mặc định là chưa đăng nhập
      // String loginStatus = "Dang xuat";  // Mặc định trạng thái là đăng xuất
    ...
    // Delay 5 giây trước khi ghi dữ liệu tiếp theo
...
void taskRFID(void* parameter) {
  for (;;) {
    // Kích hoạt RFID
    // digitalWrite(SD_CS, HIGH);   // Tắt SD
    // digitalWrite(SS_PIN, LOW);  // Bật RFID
  ...
  if (xSemaphoreTake(buzzerMutex, portMAX_DELAY)) {
    if (user->isLoggedIn && memcmp(user->uid, lastLoggedUID, 4) == 0) {
      ...
      appendLog(currentDate, currentTime, user->name, "Dang nhap", currentLat, currentLon,
                ...
                overspeedCount, sleepGucCount, collisionCount, "");  // Ghi log trạng thái đăng xuất
      ...
      appendLog(currentDate, currentTime, user->name, "Dang nhap", currentLat, currentLon,
                currentSatellites, currentHDOP, GSMlevel, speed, drivingTimes, voltageInput, bat_percentage,
                overspeedCount, sleepGucCount, collisionCount, "");  // Ghi log trạng thái đăng nhập
      sendSMS(alertphoneNumber, generateReportMessage().c_str());
      sendSMS(alertphoneNumber, generateAlertMessage().c_str());
    }
    xSemaphoreGive(buzzerMutex);
...
void buzzerCollisionTask(void* pvParameters) {
  while (true) {
    // Sử dụng semaphore để kiểm soát buzzer
    if (xSemaphoreTake(buzzerMutex, portMAX_DELAY)) {
      if (totalAcceleration >= LIGHT_COLLISION_THRESHOLD && totalAcceleration < MODERATE_COLLISION_THRESHOLD) {
        // Cảnh báo va chạm nhẹ
        playBuzzerTone(5, 100, 1000);
      } else if (totalAcceleration >= MODERATE_COLLISION_THRESHOLD && totalAcceleration < SEVERE_COLLISION_THRESHOLD) {
        // Cảnh báo va chạm trung bình
        playBuzzerTone(5, 100, 1500);
      } else if (totalAcceleration >= SEVERE_COLLISION_THRESHOLD) {
        // Cảnh báo va chạm nghiêm trọng
        playBuzzerTone(10, 100, 2000);
      }
      // Giải phóng semaphore
      xSemaphoreGive(buzzerMutex);
    }
    // Đợi trước khi đọc giá trị tiếp theo
    vTaskDelay(200 / portTICK_PERIOD_MS);
  }
}
...
String generateReportMessage() {
  ...
  if (!vehiclestatus) {
    reportMessage += "- Key status : OFF\n";
  } else {
    reportMessage += "- Key status : ON\n";
  }
